...
<div class="page-header">
    <h2><?= e($silo['nombre']) ?> <span style="font-weight:400;color:#6b7280;font-size:1rem">— <?= e($silo['granja_nombre']) ?></span></h2>
...
<div class="detail-header" style="margin-bottom:1.5rem">
    <div class="detail-meta">
        <div class="meta-item">
            <strong style="color:<?= $alerta ? '#dc2626' : 'inherit' ?>"><?= number_format($silo['stock_actual_kg'], 0) ?> kg</strong>
            Stock actual
        </div>
        <div class="meta-item">
    ...
    <div style="text-align:center;min-width:120px">
        ...
        <div style="font-size:.78rem;color:#9ca3af;margin:.3rem 0 .5rem">nivel actual</div>
        <div style="height:10px;background:#e5e7eb;border-radius:99px;overflow:hidden">
            <div style="height:10px;width:<?= min($pct,100) ?>%;background:<?= $color ?>;border-radius:99px;transition:width .4s"></div>
        </div>
        <div style="margin-top:.6rem">
            <a href="<?= base_url("silos/{$silo['id']}/editar") ?>" class="btn btn-secondary btn-sm">Editar silo</a>
        </div>
    </div>
</div>
...
<div class="list-card">
    ...
    <div style="padding:1rem;display:grid;grid-template-columns:1fr 1fr;gap:1rem">
        <div class="kpi-card">
            ...
            <div class="kpi-value"><?= number_format($proy['consumo_diario_kg'], 0) ?> kg</div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:.8rem">
        ...
        <tbody>
            ...
            <tr style="background:<?= $i%2===0?'#fff':'#f9fafb' ?>;border-bottom:1px solid #f3f4f6">
                ...
                <td style="padding:.35rem .75rem;text-align:right;font-weight:600"><?= number_format((float)($l['consumo_diario_kg']??0), 0) ?> kg</td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
...
<div style="display:flex;flex-direction:column;gap:1rem">
    ...
    <div class="form-card">
        ...
        <form method="POST" action="<?= base_url("almacen/{$silo['id']}/recarga") ?>">
            <?= csrf_field() ?>
    ...
    <div class="form-card">
        ...
        <form method="POST" action="<?= base_url("almacen/{$silo['id']}/pedido") ?>">
            <?= csrf_field() ?>
            <div class="form-group">
    ...
    <table class="list-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <form method="POST" action="<?= base_url("almacen/{$silo['id']}/recarga/{$r['id']}/eliminar") ?>"
