<div class="legal-page">
    <h1>Aviso legal</h1>
    <div class="legal-sub">Última actualización: <?= date('d/m/Y') ?></div>

    <p>
        En cumplimiento de la <strong>Ley 34/2002, de 11 de julio, de Servicios de la Sociedad de la
        Información y de Comercio Electrónico (LSSI-CE)</strong>, se facilita a continuación la
        información legal relativa al sitio web <a href="<?= base_url() ?>"><?= e(parse_url(base_url(), PHP_URL_HOST) ?? 'granja.baltae.com') ?></a>
        (en adelante, &laquo;la Plataforma&raquo;).
    </p>

    <h2>1. Titularidad</h2>
    <table>
        <tr><th>Titular</th><td>BALTAE Soluciones Ganaderas</td></tr>
        <tr><th>Contacto</th><td><a href="mailto:notificaciones@baltae.com">notificaciones@baltae.com</a></td></tr>
        <tr><th>Sitio web</th><td><a href="<?= base_url() ?>"><?= e(base_url()) ?></a></td></tr>
    </table>

    <h2>2. Objeto</h2>
    <p>
        La Plataforma es una herramienta SaaS de gestión integral de explotaciones ganaderas: granjas,
        naves, cuadras, lotes, movimientos, pesajes, almacén, inventarios e informes. El acceso al
        servicio requiere registro previo del usuario.
    </p>

    <h2>3. Condiciones de uso</h2>
    <p>
        El usuario se compromete a hacer un uso correcto de la Plataforma, conforme a la legalidad
        vigente, la buena fe y el orden público. Queda prohibido cualquier uso con fines ilícitos,
        lesivos de derechos de terceros, o que pueda dañar, sobrecargar o deteriorar el servicio.
    </p>
    <p>
        El usuario es responsable de la veracidad de los datos que introduce en la Plataforma, así como
        de mantener la confidencialidad de sus credenciales de acceso.
    </p>

    <h2>4. Propiedad intelectual e industrial</h2>
    <p>
        Todos los contenidos de la Plataforma (textos, código fuente, diseño, logotipos, marcas e
        imágenes) son titularidad de BALTAE o de terceros que han autorizado su uso, y están protegidos
        por la legislación nacional e internacional sobre propiedad intelectual e industrial.
    </p>

    <h2>5. Exclusión de responsabilidad</h2>
    <p>
        BALTAE no garantiza la disponibilidad y continuidad ininterrumpida del servicio, ni se
        responsabiliza de los daños que pudieran derivarse de fallos técnicos, interrupciones, virus o
        cualquier otra causa ajena a su control. La Plataforma se ofrece &laquo;tal cual&raquo; sin
        garantías expresas o implícitas más allá de las legalmente exigibles.
    </p>

    <h2>6. Legislación aplicable y jurisdicción</h2>
    <p>
        El presente aviso legal se rige por la legislación española. Para cualquier controversia
        relacionada con el uso de la Plataforma, las partes se someten a los Juzgados y Tribunales del
        domicilio del titular, salvo cuando la normativa aplicable disponga otro fuero imperativo.
    </p>
</div>
